...
All findings share the `Finding` shape (`src/core/types.ts`): `ruleId`, `category` (`secret | injection | auth | dependency`), `severity` (`critical | high | medium | low`), `title`, `description`, `fixSuggestion`, `file`, `line`, `column?`, `snippet`. `runScan` merges findings from all scanners and sorts by severity, then file, then line.
...
- **New regex-based pattern** (injection/auth/secret-like-but-generic): add an entry to `rules/patterns.yaml`. No TypeScript changes required.
